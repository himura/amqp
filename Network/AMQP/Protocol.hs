...
peekFrameSize = runGet f
  where
    f = do
        _frameType <- getWord8 -- 1 byte
        _channel <- get :: Get ChannelID -- 2 bytes
...
getPayload 1 _payloadSize = do --METHOD FRAME
    payLoad <- get :: Get MethodPayload
    return (MethodPayload payLoad)

getPayload 2 _payloadSize = do --content header frame
